...
# Lista explícita para que el mensaje de error muestre siempre todos los formatos (incl. .mpeg, .mpg)
ALLOWED_EXTENSIONS_LIST = [".ogg", ".opus", ".mp3", ".mpeg", ".mpg", ".wav", ".m4a", ".flac"]
...
@app.get("/allowed-formats")
async def allowed_formats():
    """Devuelve la lista de extensiones de audio permitidas (incl. .mpeg, .mpg)."""
    return {"extensions": ALLOWED_EXTENSIONS_LIST}
...
async def transcribe_audio(
    ...
    if not validate_audio_file(file.filename):
        raise HTTPException(
            status_code=400,
            detail=f"Formato de archivo no soportado. Formatos permitidos: {', '.join(ALLOWED_EXTENSIONS_LIST)}"
        )
