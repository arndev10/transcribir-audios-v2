...
function AudioUploader({ onTranscription, onError, onLoading, onProgress, isLoading, progress }) {
  ...
  const [allowedExtensions, setAllowedExtensions] = useState(FALLBACK_EXTENSIONS)
    ...
    fetch('http://localhost:8000/allowed-formats')
      ...
      .then(data => {
        if (data.extensions && Array.isArray(data.extensions) && data.extensions.length > 0) {
          setAllowedExtensions(data.extensions)
        }
      })
      .catch(() => {})
  }, [])
  ...
  const handleFileSelect = (file) => {
    if (file) {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        onError(`Formato no soportado. Formatos permitidos: ${allowedExtensions.join(', ')}`)
        return
      }
